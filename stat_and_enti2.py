...
def top_ten_authors(tmp, followers, user_name, nickname):
    ...
    for i in tmp:
        if i[followers] != '':
            follow_lst.append(int(i[followers]))
...
def top_ten_tweets(tmp, rts, tweet, author):
    """Top 10 Tweets ,Authors and count Retweets"""
    d = {}
...
def full_column(tmp, column):
    """"Returns full column as one text"""
    full_text = []
    for i in tmp:
